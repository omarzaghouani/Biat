    ...
    public function front(TransportRepository $transportRepository, Request $request): Response
{
    ...
    #[Route('/transport/{num_t}/edit', name: 'app_transport_edit', methods: ['GET', 'POST'])]
...
public function events(TransportRepository $transportRepository): Response
{
    ...
    foreach ($transports as $transport) {
        $events[] = [
            'title' => $transport->getNumT(), // Titre de l'événement
            'start' => $transport->getDd()->format('Y-m-d\TH:i:s'), // Date de début de l'événement
            'end' => $transport->getDa()->format('Y-m-d\TH:i:s'), // Date de fin de l'événement
        ];
    }
...
public function servicesPlusAchetes(TransportRepository $transportRepository): Response
{
    ...
    foreach ($mostPurchasedTransports as $transport) {
        $stats[] = [
            'type' => $transport['num_t'], 
            'count' => $transport['cout'], 
        ];
    }
...
public function calendarEvents(TransportRepository $transportRepository): JsonResponse
{
    $transports = $transportRepository->findAll();
    ...
    foreach ($transports as $transport) {
        $events[] = [
            'title' => 'Transport ' . $transport->getNumT(), // Titre de l'événement
            'start' => $transport->getDd()->format('Y-m-d'), // Date de début de l'événement
            'end' => $transport->getDa()->format('Y-m-d'), // Date de fin de l'événement
